...
class Task:
    def __init__(self, id: str, duration: int, start_time: int, processor: int):
        self.id = id
...
def asap_binding(graph: DiGraph) -> dict[str,int]:
    asap_times: dict[str,int] = {}
...
def modified_critical_path(graph: DiGraph, processors: ProcessorsAvailability, mem_lim: int,
    ...
    schedule: list[Task] = list(task_map.values())
